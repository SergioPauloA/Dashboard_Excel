...
export default function Dashboard() {
  ...
  const valorFaturadoPorMes = useCallback(() => {
    ...
    return months.map(month => {
      ...
      return {
        name: monthNames[month - 1],
        valor: valorFaturado,
        quantidade: monthData.length // Adicionado para o gráfico de Valor Faturado vs. Procedimentos
      };
    });
  }, [filteredData]);
  ...
  const calcularEstatisticas = useCallback(() => {
    ...
    // Modificado: Agora conta procedimentos com status "A faturar"
    const procedimentosAFaturar = filteredData.filter(item => item['status pedido'] === "A faturar");
    const valorFaltaFaturar = procedimentosAFaturar.length;
  ...
  // CustomTooltip modificado para formatar todos os valores monetários
      ...
      return (
        <div className="custom-tooltip">
          ...
          {payload.map((entry, index) => {
            // Verificar se o nome da propriedade indica um valor monetário
            const isMonetary = entry.name.toLowerCase().includes("valor") || 
                               entry.dataKey === "valor" ||
                               entry.name.includes("Faturado");
            
            return (
              <p key={index} style={{ color: entry.color }}>
                {entry.name}: {isMonetary 
